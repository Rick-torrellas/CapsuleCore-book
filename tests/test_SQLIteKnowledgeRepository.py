...
class TestSQLiteKnowledgeRepository:
    ...
    def test_find_by_tag_search(self, repo, sample_entry):
        ...
        entry2 = Entry(
            id="entry-456", title="Otro", content="...", 
            tags=["sql"], created_at=datetime.now()
        )
        repo.save(entry2)
    ...
    def test_delete_cascades_relations(self, repo, sample_entry):
        # Crear dos entradas
        entry2 = Entry(id="entry-2", title="B", content="...", tags=[], created_at=datetime.now())
        repo.save(sample_entry)
        repo.save(entry2)
        
        # Crear relación
        rel = Relation(from_id=sample_entry.id, to_id=entry2.id, connection_type="links")
        ...
        with sqlite3.connect(repo.db_path) as conn:
            res = conn.execute("SELECT * FROM relations WHERE from_id = ?", (sample_entry.id,)).fetchall()
            assert len(res) == 0
