...
class InMemoryRepository(KnowledgeRepository):
    ...
    def save(self, entry): self.entries[entry.id] = entry
    def get_by_id(self, entry_id): return self.entries.get(entry_id)
    ...
    def remove_relation(self, relation): self.relations.remove(relation)
